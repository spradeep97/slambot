...
public:
    ...
    bool isValid(Node& n);
    bool isFree(Node& n);
    bool is_safe_cell(Node& n);
    bool isGoal(Node& n);
    void makePath(Node& n);
    double calc_h_cost(Node& n);
    double calc_proximity_cost(Node& n);
    void searchPath(void);
    std::vector<Node> expand(Node& n);
    void updateKid(Node& kid, Node& parent);

    //Debugging functions
    void print_closedList(void);
    void print_node(Node& n);
    void print_path(void);


    Node start_, goal_;
    std::set<Node> openList_;
    bool **closedList_;
    Node **nodeGrid_;
    bool foundDest_;
    robot_path_t path_;
