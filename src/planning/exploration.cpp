...
double dist(pose_xyt_t& pose1, pose_xyt_t& pose2);
...
, pathReceived_(false)
{
    ////std::cout << "DEBUG: STATUS: ---Initializing---" << std::endl;
    assert(lcmInstance_);   // confirm a nullptr wasn't passed in
    //std::cout << "DEBUG: Subscribing to handleMap, handlePose, handleConfirmation." << std::endl;
    ...
    //std::cout << "DEBUG: Creating status message to be published." << std::endl;
    ...
    //std::cout << "DEBUG: Setting up motion planner parameters." << std::endl;
...
bool Exploration::exploreEnvironment()
{
        ...
        && (state_ != exploration_status_t::STATE_FAILED_EXPLORATION))
    {
        ...
        if(isReadyToUpdate())
        {
            //std::cout << "DEBUG: Exploration loop:" << debug_i << std::endl;
            runExploration();
            ++debug_i;
        }
...
{
    //std::cout << "DEBUG: copying data for update." << std::endl;
    std::lock_guard<std::mutex> autoLock(dataLock_);
...
{
    //std::cout << "DEBUG: executing state machine." << std::endl;
...
int8_t Exploration::executeInitializing(void)
{
    ...
    //std::cout << "DEBUG: Execute Initializing." << std::endl;
    exploration_status_t status;
    status.utime = utime_now();
    status.team_number = teamNumber_;
    status.state = exploration_status_t::STATE_INITIALIZING;
...
int8_t Exploration::executeExploringMap(bool initialize)
{
    ...
    */
    //std::cout << "DEBUG: Execute exploring map." << std::endl;
    if (initialize) {
        //TODO initialize local variables here.
        frontiers_ = find_map_frontiers(currentMap_, currentPose_);
        planner_.setMap(currentMap_);
        std::cout << "Planning" << currentPose_.x << "|" << currentPose_.y << "||" << \
        lastPose_.x << "|" << lastPose_.y << std::endl;
        currentPath_ = plan_path_to_frontier(frontiers_, currentPose_, currentMap_, planner_);
        pathReceived_ =  true; //Should I do this?
        lastPose_ = currentPose_;
    }
    if (dist(currentPose_, lastPose_) > 3 * currentMap_.metersPerCell()) {
        frontiers_ = find_map_frontiers(currentMap_, currentPose_);
        planner_.setMap(currentMap_);
        std::cout << "Planning" << currentPose_.x << "|" << currentPose_.y << "||" << \
        lastPose_.x << "|" << lastPose_.y << std::endl;
