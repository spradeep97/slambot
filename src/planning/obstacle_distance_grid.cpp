...
void ObstacleDistanceGrid::setDistances(const OccupancyGrid& map)
{
    ...
    if (is_empty) {
      ...
      for(cell.y = 0; cell.y < heightInCells(); ++cell.y)
      {
          for(cell.x = 0; cell.x < widthInCells(); ++cell.x)
          {
              distance(cell.x, cell.y) = 9999.0;
          }
      }
    }
...
bool ObstacleDistanceGrid::is_cell_free(int x, int y)
{
    if (is_empty) {
        return true;
    }
    return (std::abs(distance(x, y)) > 1E-6 );
}
