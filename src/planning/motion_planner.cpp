...
bool MotionPlanner::isPathSafe(const robot_path_t& path) const
{
    ...
    for (auto& n : path.path) {
        p_float.x = n.x;
        p_float.y = n.y;
        p_int = global_position_to_grid_cell(p_float, distances_);
        if (distances_(p_int.x, p_int.y) < searchParams_.maxDistanceWithCost) {
            return false;
        }
    }
    return true;
...
void MotionPlanner::setParams(const MotionPlannerParams& params)
{
    ...
    //std::cout << "DEBUG: Finished setting planner params:" << std::endl;
    std::cout << searchParams_.minDistanceToObstacle <<"|"<< searchParams_.maxDistanceWithCost \
    <<"|"<<searchParams_.distanceCostExponent << std::endl;
}
