...
int main(int argc, char** argv)
{
    ...
    //std::cout << "DEBUG: LCM Instance created" << std::endl;

    if(!lcmInstance.good()) return 1;

    //std::cout << "DEBUG: Creating exploration instance." << std::endl;
    Exploration exploration(teamNumber, &lcmInstance);
    //std::cout << "DEBUG: Exploration instance Instance created" << std::endl;
    ...
    std::thread exploreThread([&exploration, &explorationComplete]() {
        //std::cout << "DEBUG: Launching exploration thread." << std::endl;
